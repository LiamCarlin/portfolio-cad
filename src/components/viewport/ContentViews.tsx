...
// Drawing View - Technical drawings and CAD views
export function DrawingView({ project, lightMode }: { project: Project; lightMode: boolean }) {
  const bgClass = lightMode ? 'bg-gray-50' : 'bg-gray-900';
  const textClass = lightMode ? 'text-gray-900' : 'text-white';
  const mutedClass = lightMode ? 'text-gray-500' : 'text-gray-400';

  // Find images that might be drawings/technical docs
  const drawings = project.images?.filter(img => 
    img.name.toLowerCase().includes('drawing') || 
    img.name.toLowerCase().includes('cad') ||
    img.name.toLowerCase().includes('technical')
  ) || [];

  // Also check content blocks for images
  const contentImages = project.contentBlocks?.filter(b => 
    ...
    <div className={`h-full overflow-y-auto ${bgClass} p-6`}>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className={`text-2xl font-bold ${textClass}`}>Technical Drawings</h2>
        </div>

        {project.links.onshape && (
          <div className={`${lightMode ? 'bg-white' : 'bg-gray-800'} rounded-xl p-6 mb-6 shadow-md`}>
            <h3 className={`font-semibold ${textClass} mb-3`}>CAD Document</h3>
            <p className={`${mutedClass} mb-4`}>View the full CAD model in Onshape:</p>
            <a
              href={project.links.onshape}
              target="_blank"
              rel="noopener noreferrer"
        ...
        {drawings.length > 0 ? (
          <div className="grid md:grid-cols-2 gap-6">
            {drawings.map((img) => (
              <div key={img.id} className={`${lightMode ? 'bg-white' : 'bg-gray-800'} rounded-xl overflow-hidden shadow-md`}>
                <img
                  src={img.data}
                  alt={img.name}
                ...
                />
                <div className="p-4">
                  <h4 className={`font-medium ${textClass}`}>{img.name}</h4>
                  {img.caption && <p className={`text-sm ${mutedClass}`}>{img.caption}</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className={`text-center py-16 ${mutedClass}`}>
            ...
            <p className="text-sm">Upload drawings in the admin panel's Images tab.</p>
            {project.links.onshape && (
              <p className="text-sm mt-2">You can view the CAD model using the link above.</p>
            )}
          </div>
        )}
      </div>
